...
word_locs = np.zeros_like(words_orig, dtype=np.intp)

res_i = 0
i = 0
j = 0

while i < words.shape[0] and j < words_orig.shape[0]:
    while j < words_orig.shape[0] and words_orig[j] < words[i]:
        j += 1
    if j >= words_orig.shape[0]:
        break
    while i < words.shape[0] and words[i] < words_orig[j]:
        i += 1
    if i < words.shape[0] and words[i] == words_orig[j]:
        word_locs[res_i] = i
        res_i += 1
        i += 1
